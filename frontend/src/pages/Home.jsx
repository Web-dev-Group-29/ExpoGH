      ...
      <section className="home-hero">
        ...
        <div className="home-hero-content">
          ...
          <div className="home-hero-actions">
            <Link to="/regions" className="home-hero-btn">Explore Regions</Link>
            <Link to="/experiences" className="home-hero-btn">View Destination</Link>
          </div>
        </div>
      ...
      <section className="home-filter">
        <div className="home-filter-inner">
          <div className="home-filter-row">
            ...
            {CATEGORIES.map((cat) => (
              <Link key={cat} to={`/regions?category=${cat}`} className="home-filter-chip">{cat}</Link>
            ))}
          </div>
          <div className="home-filter-row" style={{ paddingTop: '0.5rem' }}>
            <span className="home-filter-label">REGION</span>
            {REGIONS_LIST.map((r) => (
              <Link key={r} to={`/regions?region=${encodeURIComponent(r)}`} className="home-filter-chip">{r}</Link>
            ))}
          </div>
        </div>
      </section>
